...
async fn root() -> impl Responder {
    HttpResponse::Ok().body("Funcionando")
}
...
/* use actix_web::{post, web, App, HttpServer, HttpResponse, middleware::Logger};
...
#[post("/tweet")]
async fn receive_tweet(tweet: web::Json<Tweet>) -> HttpResponse {
    println!("Received tweet: {:?}", tweet);
    // En un flujo real: mandar por gRPC al servicio Go o publicar en Broker
    HttpResponse::Ok().body("{\"status\":\"received\"}")
}

#[actix_web::main]
async fn main() -> std::io::Result<()> {
    env_logger::init_from_env(env_logger::Env::default().default_filter_or("info"));
    println!("Starting API Rust on 0.0.0.0:8080");
    HttpServer::new(|| App::new().wrap(Logger::default()).service(receive_tweet))
        .bind(("0.0.0.0", 8080))?
        .run()
        .await
}
